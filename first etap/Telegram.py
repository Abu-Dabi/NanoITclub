...
def website(message):
    ...
    osman = types.KeyboardButton('/Osman')
    ...
    markup.add(osman, vinland_saga, my_hero_academia, blue_lock)
    bot.send_message(message.chat.id, 'Click on the button', reply_markup=markup)


@bot.message_handler(commands=['Osman'])
def website(message):
    markup = types.InlineKeyboardMarkup()
    markup.add(
        types.InlineKeyboardButton("Go to watch anime", url="https://www.ts.kg/show/kurulus_osman"))
...
def get_user_text(message):
    if message.text == 'Токийские мстители':
        bot.send_message(message.chat.id, 'True! Tokyo avengers the best anime', parse_mode='html')
    else:
        bot.send_message(message.chat.id, "False! Click on /watch to go watch best anime", parse_mode='html')
